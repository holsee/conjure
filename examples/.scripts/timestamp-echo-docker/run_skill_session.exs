#!/usr/bin/env elixir
#
# Example: Conjure.Session.chat with Docker execution
#
# Demonstrates using Conjure.Session API with Docker sandboxed execution.
# The mock Claude reads the skill and uses relative paths as documented.
#
# Prerequisites:
...
IO.puts("=== Conjure Docker Session Example ===")
IO.puts("Message: #{message}\n")

# Check Docker
case Conjure.Executor.Docker.check_environment() do
...
case Conjure.Loader.load_skill_file(skill_file) do
  {:ok, skill} ->
    ...
    {:ok, session} = Session.new_docker([skill], timeout: 30_000)
